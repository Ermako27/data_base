...
s1 = p.readline()
while s1:
    Id,Name,Sex,Age,Height,Weight,Team = s1[:-1].split('|')
...
s2 = g.readline()
while s2:
    Id, year, season, city = s2[:-1].split('|')
...
s3 = e.readline()
while s3:
    Id,Sport,Event,PCount = s3[:-1].split('|')
...
s4 = pe.readline()
while s4:
    Id,participant_id,event_id = s4[:-1].split('|')
...
s5 = eg.readline()
while s5:
    Id,game_id,event_id = s5[:-1].split('|')
...
s6 = gp.readline()
while s6:
    Id,participant_id,game_id = s6[:-1].split('|')
